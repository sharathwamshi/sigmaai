...
ONBOARDING_QUESTIONS = {
    "purpose": {
        "question": "🎯 *Question 1 of 5* /n What can I help you with today?",
        "options": [
            ("🛍 Buy Products", "buy_products"),
    ...
    "category": {
        "question": "📦 *Question 2 of 5* /n Which product category are you interested in?",
        "options": [
            ("📷 Still Lenses", "Still Lenses"),
    ...
    "usage": {
        "question": "🎥 *Question 3 of 5* /n What will you use it for?",
        "options": [
    ...
    "budget": {
        "question": "💰 *Question 4 of 5* /n What is your approximate budget?",
        "options": [
            ("₹ Under 50K", "Under 50K"),
            ("₹ 50K – 1L", "50K-1L"),
            ("₹ 1L – 3L", "1L-3L"),
    ...
    "location": {
        "question": "📍 *Question 5 of 5* /n Which city are you located in?",
        "options": [
            ("🌆 Bangalore", "Bangalore"),
            ("🏙 Chennai", "Chennai"),
            ("🌃 Mumbai", "Mumbai"),
...
PURPOSE_FLOW = {
    "buy_products": ["category", "usage", "budget", "location"],
    "check_prices": ["category"],
    "find_dealer": ["location"],
    "service_repair": ["location"],
    "contact_sigma": [],
}
...
async def route_after_onboarding(q, ctx, user):
    ...
    if purpose == "buy_products":
        await send(q, "🛍 *Based on your requirements, here are our product categories:*", categories_kb())
    elif purpose == "check_prices":
        await send(q, "💰 *Pricing Information*\n\nSelect a category to view prices.", prices_category_kb())
    elif purpose == "find_dealer":
        ...
        if dealers:
            ...
            rows.append(_row(("🏠 Main Menu", "menu:main")))
            await send(q, f"🤝 *Authorised Dealers in {location}*\n\nFound *{len(dealers)}* dealers.",
                       InlineKeyboardMarkup(rows))
        else:
            await send(q, f"No dealers found in *{location}*. Showing all regions.", dealer_regions_kb())
    elif purpose == "service_repair":
        location = answers.get("location", "")
        centers = search_service_centers(location)
        if centers:
            ctx.user_data["service_centers"] = centers
            await send(q, f"🔧 *Service Centres in {location}*", service_list_kb(centers))
        else:
            await send(q, f"🔧 No exact service centre found in {location}. Showing all centres.",
                       kb([("🔧 View All Centres", "menu:service")]))
    elif purpose == "contact_sigma":
        await send(q, CONTACT_MSG, kb([("🏠 Main Menu", "menu:main")]))
