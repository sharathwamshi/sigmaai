...
def build_system_prompt(detected_language=None):
    ...
    dealer_summary = []
    for sh, label in DEALER_SHEET_MAP.items():
        dealers = load_dealers(sh).get(sh, [])
        cities  = list({d.get("city","").strip() for d in dealers if d.get("city","").strip()})[:5]
        dealer_summary.append(f"  {label}: {len(dealers)} dealers in {', '.join(cities)} and more")

    return f"""You are {brand} — expert AI product guide for Sigma ({site_url}).
{lang_inst}
{length_map.get(cfg.get('response_length','medium'),'')} {style_map.get(cfg.get('response_style','professional'),'')}
...
{('LOYALTY PROGRAM (mention when relevant): ' + loyalty) if loyalty else ''}

PRODUCT CATALOGUE ({len(all_p)} products):
{chr(10).join(cat_lines)}

DEALER NETWORK (1215+ authorised dealers across India):
{chr(10).join(dealer_summary)}
Direct users to sigmaindia.in/dealer-network/ for dealer lookup by city/state.

FORMAT — when recommending products output this exact block:
```json_products
[{{"name":"...","line":"...","category":"...","focal_length":"...","aperture":"...",
   "mounts":"...","key_features":"...","url":"{site_url}",
   "amazon_in":"https://www.amazon.in/s?k=Sigma+product+name",
   "price_inr":"₹X","price_range_inr":"₹X–₹Y","reason":"why this matches"}}]
```
Then write explanation after the block.

{'Include ```json_prices``` with Sigma India, Amazon India, Flipkart when user asks about pricing.' if show_price else 'Do NOT include price comparison tables.'}

{'Include ```json_dealers``` block when user asks about dealers/where to buy.' if show_deal else 'Do NOT include dealer sections.'}

{'Do NOT include Amazon links.' if not show_amz else ''}

PRICE BLOCK FORMAT:
```json_prices
[{{"platform":"Sigma India","price":"₹X","url":"{site_url}","status":"Authorised"}},
 {{"platform":"Amazon India","price":"₹X","url":"https://www.amazon.in/s?k=sigma","status":"Check listing"}},
 {{"platform":"Flipkart","price":"₹X","url":"https://www.flipkart.com/search?q=sigma","status":"Check listing"}}]
```

DEALER BLOCK FORMAT:
```json_dealers
[{{"name":"Sigma India Network","type":"1215+ Authorised Dealers","location":"Pan India","url":"https://sigmaindia.in/dealer-network/"}},
 {{"name":"Amazon India","type":"Online Retailer","location":"Pan India","url":"https://www.amazon.in/s?k=sigma+lens"}}]
```

NEVER invent products Sigma does not make."""
...
def chat():
    ...
    # ── FIX: inject images into product cards ──────────────
    # Build fresh map from current disk state (not cached)
    img_map = {}
    for p in get_all_products():
        name = p.get("Product Name", "")
        pid  = p.get("_id", _product_id(name))
        img  = _find_image(pid)   # always re-check disk
        img_map[name] = img
    ...
    for prod in parsed["products"]:
        ...
        # if not found, try ID-based lookup
        if not img:
            pid = _product_id(prod_name)
            img = _find_image(pid)
        prod["_image"] = img
