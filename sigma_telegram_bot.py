...
async def send_onboarding_question(update_or_query, step: int):
    q = ONBOARDING_QUESTIONS[step]
    if hasattr(update_or_query, "message"):
        await update_or_query.message.reply_text(
            q["question"],
            parse_mode="Markdown",
            reply_markup=onboarding_kb(step),
        )
    else:
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["onboarding_step"] = 0
    await update.message.reply_text(
        f"👋 *Welcome, {user.first_name}!* \n Before we begin, please answer a few quick questions so I can guide you better.",
        parse_mode="Markdown",
    )
    await send_onboarding_question(update, 0)
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("onboard:"):
        ...
        step = int(step_str)
        ctx.user_data.setdefault("onboarding_answers", {})[
            ONBOARDING_QUESTIONS[step]["key"]
        ] = value
        await q.answer(f"Selected: {value}")
        try:
        ...
        if next_step < len(ONBOARDING_QUESTIONS):
            ctx.user_data["onboarding_step"] = next_step
            await send(q, ONBOARDING_QUESTIONS[next_step]["question"], onboarding_kb(next_step))
        else:
            ctx.user_data["onboarding_complete"] = True
            await send(
                q,
