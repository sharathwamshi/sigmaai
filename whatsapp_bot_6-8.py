...
- List-style menus (tap-to-select tiles) are still used wherever the
  user is choosing between several named options (e.g. product
  category, state). These are sent via Twilio's twilio/list-picker
  content type.
...
- All the URLs, phone numbers and email addresses marked
  ...
  live.
- A plain-text menu is used only as an automatic fallback if the
  interactive message API call itself fails (network/account issue).
...
def send_menu(to: str, header: str, body_text: str, options: list,
    ...
    """
    Sends the options as real WhatsApp buttons (twilio/quick-reply),
    stacked directly in the chat — no "Choose an option" popup.
    WhatsApp allows a max of 3 buttons per message, so option lists
    longer than 3 are split across consecutive button messages; the
    first carries body_text, any further ones a short "More options:"
    lead-in, so the whole set still reads as one stacked menu.
...
MAIN_OPTIONS = [
    ...
    ("Sigma Loyalty Program (SIGMA Focal Circle)", "loyalty"),
]
...
LOYALTY_BODY = (
    "Join the loyalty program in 3 simple steps.\n\n"
    "1. Visit the link\n"
    "2. Create account / login\n"
...
def route(phone: str, user_name: str, key: str, sess: dict):
    ...
    # -- Browse Products ---------------------------------------------------
        ...
        if info:
            back = [(info["label"], key), ("Categories", "products"), ("Main Menu", "main")]
            sess["options"] = back
